...
@section('content')
    <div class="page-head">
        ...
        <div class="actions">
            ...
            @endif
            @if ($nota->can_reject)
                <form method="post" action="{{ route('fiscal.notas.reject', $nota->id) }}" onsubmit="return confirm('Rejeitar esta nota fiscal?')">
                    @csrf
                    <button class="btn danger" type="submit">Rejeitar nota</button>
                </form>
            @endif
        </div>
    </div>
